...
{
	LOG_HIHOOK;
	BOOL valid = (!F_Multiplayer() && FOptions.resources_overflow && P_Main->GetPlayer() == h3_ActivePlayer);
	H3Resources backup;
	if (valid)
		backup = h3_ActivePlayer->playerResources;

	const int r = THISCALL_2(int, h->GetDefaultFunc(), This, msg);

	if (valid)
	{
		H3Resources& res = h3_ActivePlayer->playerResources;
